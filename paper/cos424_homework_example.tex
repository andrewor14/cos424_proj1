...
We used the Python NLTK library to tokenize, lower case, lemmatize and stem each word using the Porter stemming methods. In addition to individual word tokens, we also include bigrams in our bag of "words" representation. Each entry in our feature vector then is the number of times each "word" appears in a document.
...
Each tree can be built as follows using the ID3 algorithm []:
\begin{enumerate}
\item Select the best feature by information gain
\item Split input data 
\end{enumerate}
